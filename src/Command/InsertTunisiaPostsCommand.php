...
class InsertTunisiaPostsCommand extends Command
{
    protected static $defaultName = 'app:insert-tunisia-posts';
    protected static $defaultDescription = 'Insert 10 Tunisia-related posts with comments and reactions';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Inserting Tunisia-related posts');

        // Get users from the database
        $users = $this->entityManager->getRepository(User::class)->findAll();

        if (empty($users)) {
            $io->error('No users found in the database. Please create at least one user first.');
            return Command::FAILURE;
        }

        // Use the first user as the author
        $author = $users[0];
        $io->note(sprintf('Using user "%s %s" as the author', $author->getFirstName(), $author->getLastName()));
        ...
        $progressBar = $io->createProgressBar(count($tunisiaPosts));
        $progressBar->start();
        ...
        foreach ($tunisiaPosts as $index => $postData) {
            ...
            $this->entityManager->flush(); // Flush to get the post ID
        ...
        $progressBar->finish();
        $io->newLine(2);
        $io->success('Successfully inserted 10 Tunisia-related posts with comments and reactions!');
